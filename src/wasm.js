 ...
 */
async function svgToPng_(svgString) {
  const wasm = await import("./pkg/example_bg.wasm");
  const { __wbg_set_wasm, svg_to_png } = await import("./pkg/example_bg.js");

  __wbg_set_wasm(wasm);

  const result = svg_to_png(svgString);
...
globalThis.svgToPng_ = svgToPng_;
